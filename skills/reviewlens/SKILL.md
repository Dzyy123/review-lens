...
**Layer 2 逐条详解** —— 每条按下面结构。**三条硬规则**：① 标题前缀必须带类型对应的**颜色 emoji**（🟢⚪🔵🟠🟣🔴）；② 必须先**逐字摘录** reviewer 这条实际写的句子（带引号，不要改写/缩写），再分析；③ **「分析与证据」和「怎么回复」必须拆成两个独立小节**，不能合并。

```
### <颜色emoji> #编号 ｜ <一句话标题>

> **📌 Reviewer 原话**：“<逐字摘录该条 reviewer 实际写的句子>”

**🏷️ 类型**：<颜色emoji> <六类标签> — <一句话理由>

**🔎 分析与证据**
- 📄 论文依据：<论文实际写了什么，给到 节/表/附录>
- 🔬 别人做过吗 / 是否标准：<相关工作或领域惯例；不涉及可省略>
- 🎯 是否超出范围：<是/否 + 用论文目标/训练数据/baseline 举证>

**✍️ 怎么回复**
- 要不要做新实验：<不用 / 补一个小实验：具体做什么>
- 可直接粘贴（英文）：“<ready-to-paste reply>”
- 中文说明：<这句回复在做什么>
```
...
- **无论如何，强制产出一份报告文件**：只要拿到 review + 论文，就必须把完整报告写成一个 `.md` 文件交付（默认存到论文同目录或当前工作区），文件即交付物。**不要用"要不要生成报告"征询，直接生成。**
- **每条都必须带可跳转的链接与精确位置**：
  - 📄 论文依据：给 `§章节 + 表/图/算法编号 + 页码 p.N`；并给**可点击深链让读者一点直达**——本地 PDF 用 `file://<论文绝对路径>#page=N`（路径含非 ASCII 字符要 percent-encode），有公开版（如 arXiv）则附其 URL。
  - 🔬 相关工作：每个被引工作都给**可点击 URL**（arXiv / DOI / OpenReview）。
  - 第 0 节告警 / 任何"位置"引用：给**页码 + 行号**（若可得）+ 深链。
  - 报告开头放一个"**定位入口**"：论文 PDF 绝对路径 + arXiv 链接，说明所有 `p.N` 均可点击跳转。
- 聊天里只放**极简指引**：一句话说"报告已生成在 `<路径>`" + 最多 3–5 条要点摘要。**不要把整份报告复述进聊天**。
- **不要在结尾追问、不要和用户讨价还价**。唯一允许停下来发问：**缺少必要输入**（没有论文）或**会议政策不明**。其余一律先把报告做出来。
